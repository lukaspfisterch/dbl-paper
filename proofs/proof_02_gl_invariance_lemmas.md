...
By definition of $C$ (from formal definitions), $C = (\text{boundary\_version}, \text{boundary\_config\_hash}, L_{\text{rules}})$ where $L_{\text{rules}}$ is a deterministic function.
...
Since $L_{\text{rules}}$ is versioned (via $\text{boundary\_version}$) and hashed (via $\text{boundary\_config\_hash}$), any change to admission behavior produces a different $C$.

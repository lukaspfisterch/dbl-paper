...
% Build output is written to: paper/main.pdf

\maketitle

\begin{abstract}
This paper introduces Deterministic Boundary Layers (DBL), a minimal architectural model
...
are expressed exclusively as DECISION events recorded in an append-only
...
This separation yields four structural consequences: deterministic governance under
non-deterministic execution, invariance of G/L separation, replay equivalence of
normative state without re-execution, and observational non-interference.

This paper formalizes the model, states minimal axioms, and makes four claims
explicit whose proof sketches follow directly from those axioms. DBL is not a
policy framework or learning method, but an architectural pattern for accountable
AI governance, auditability, and compliance in systems where execution cannot be
made deterministic.
\end{abstract}
...
The contribution of this paper is not the derivation of complex properties from a rich dynamics. It is the identification of the minimal structural conditions under which deterministic governance, replay equivalence, and observational non-interference become unavoidable. The axioms are intentionally chosen so that these properties follow by construction. This is the point of the model: if a system satisfies these constraints, then the core guarantees are no longer optional design goals but structural consequences.

\paragraph{Relation to the Companion Execution Theory.}
This paper is layered above the companion theory \emph{Execution Without Normativity}, which isolates the execution substrate from meaning and governance. That companion paper uses its own axiom labels for execution structure only. The DBL axioms A1--A6 used here are local to the present paper and characterize the additional governance constraints required to make normativity explicit, deterministic, and replayable.
...
The paper introduces the architectural model and its axioms, states the four claims, provides proof sketches under fixed assumptions, and presents two case studies from distinct domains, followed by a related work mapping.
...
The axioms are intentionally constructed such that the claims follow by construction. The contribution is the explicit identification and formalization of these architectural constraints, not the derivation of unexpected consequences. The proofs are therefore intentionally minimal: they do not display ingenuity of derivation, but structural inevitability once A1--A6 are fixed. This section summarizes the proof ideas under assumptions A1--A6. Full lemma-based proofs are provided in the supplementary proof files.
...
This section illustrates DBL in two distinct domains to demonstrate that the architectural pattern is domain-independent. Both cases follow the same structure: boundary admission, governance decision, execution, and replay.

\subsection{LLM Prompt Governance}
...
\subsection{Financial Transaction}

A payment system processes transfer requests between accounts. Execution depends on external settlement infrastructure (banking APIs, network latency, partial failures) and is inherently non-deterministic. Regulatory requirements demand that every approval or denial is auditable and reconstructible independently of settlement outcomes.

\paragraph{Boundary Admission.}
The boundary layer $L$ receives a raw transfer request and extracts authoritative inputs: sender, recipient, amount, currency, and account status flags. Observational data such as current network latency, settlement queue depth, or previous transaction timing is excluded from $I_L$. If the request fails structural validation (for example, malformed account identifiers), it is rejected pre-ontologically.

\paragraph{Governance Decision.}
Governance evaluates $I_L$ under policy version $p_v$, which encodes transfer limits, compliance rules, and account restrictions. A DECISION event (APPROVE or DENY) is produced deterministically and appended to $V$ before any settlement is initiated. The decision depends only on $(I_L, p_v)$, not on whether the settlement infrastructure is available or how long previous transfers took.

\paragraph{Execution and Observation.}
Only if the DECISION is APPROVE does the effector initiate settlement via external banking infrastructure. The settlement result (success, timeout, partial failure), confirmation codes, and processing timestamps are recorded as EXECUTION and PROOF events. These are observational artifacts with no influence on the prior DECISION.

\paragraph{Replay and Audit.}
A compliance auditor replaying $V$ reconstructs the same DECISION sequence from $(V, I_L, C, p_v)$ without access to the settlement infrastructure, banking APIs, or original network conditions. Whether the original settlement succeeded or failed is irrelevant to the normative record.

\medskip
Both cases illustrate the same structural property: governance decisions are deterministic, recorded before execution, and replayable from authoritative inputs alone. The domain-specific details differ; the architectural guarantees do not.
...
This restriction is significant in fully distributed settings, but it still leaves a substantial practical scope. Many compliance-critical systems already enforce or approximate a single authoritative decision record, including financial ledgers, workflow engines, and audit logs. DBL is aimed at that class of systems: environments where a canonical order of governance decisions is already required, even if the underlying execution remains non-deterministic.
...
This paper introduced Deterministic Boundary Layers (DBL), an architectural model for governing non-deterministic systems with deterministic, auditable decisions. DBL separates normative decisions from observational execution by construction: governance produces DECISION events from authoritative inputs, while execution outputs are treated as non-normative observations.

This paper formalized DBL through a minimal set of axioms and made explicit that, under these assumptions, four properties follow directly: deterministic governance, invariance of G/L separation, replay equivalence of normative state, and observational non-interference. Together, these properties ensure that normative outcomes are independent of non-deterministic execution behavior and can be reconstructed and audited from an append-only event stream.

Two case studies from distinct domains---LLM prompt governance and financial transaction processing---demonstrate that the architectural guarantees are domain-independent. DBL does not address policy quality, execution determinism, or alignment of model behavior. Its contribution lies in making decisions explicit, deterministic, and replayable, thereby enabling accountability for systems that operate over inherently non-deterministic components.

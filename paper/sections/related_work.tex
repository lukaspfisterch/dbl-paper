...
DBL shares the append-only event stream as the source of truth but introduces a separation that event sourcing does not require: normative events (DECISION) are distinguished from observational events (EXECUTION, PROOF), and only the normative projection determines governance state. Event sourcing replays all events equivalently; DBL replays only the normative subset and treats the rest as non-authoritative. DBL also enforces pre-execution decisions and G/L separation.

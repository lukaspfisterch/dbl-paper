...
Event sourcing~\cite{fowler2005event} treats events as the source of truth. Commands produce events; queries read projections. CQRS~\cite{young2010cqrs} separates write and read models and formalizes event-driven architectures where state is reconstructed by replaying an ordered event log.

DBL shares the append-only event stream as the source of truth but introduces a separation that event sourcing does not require: normative events (DECISION) are distinguished from observational events (EXECUTION, PROOF), and only the normative projection determines governance state. Event sourcing records what happened. DBL records what was authorized to happen, independently of what subsequently occurred. Event sourcing replays all events equivalently; DBL replays only the normative subset and treats the rest as non-authoritative. DBL also enforces pre-execution decisions and G/L separation.
...
Reference monitors enforce access control during execution. DBL externalizes these decisions into explicit, replayable DECISION events recorded before execution. This makes governance itself inspectable and reconstructible, rather than only its runtime enforcement effects.
...
Viewed relative to the companion execution theory, DBL is a governance layer on top of a narrower execution substrate. The execution theory isolates ordered events, replay, and observation-free projection. DBL adds the additional constraints required for normative systems: authoritative admission, explicit DECISION events, pre-execution governance, and pre-ontological rejection.

...
This section presents the formal model underlying DBL. The model is minimal: it defines what must be true for the claims to hold, without prescribing implementation details.
...
where $C = (\text{boundary_version}, \text{boundary_config_hash}, L_{\text{rules}})$ is the boundary configuration.
...
Six axioms define the minimal requirements for deterministic governance.
...
\textbf{Rationale:} A0 clarifies what $V$ represents. $V$ is the normative record (what was decided), not a complete causal log (everything that happened). Absence of an event in $V$ does not imply absence of execution in the system; it implies only the absence of a normative record. L's rejections occur before $V$ construction and are not normative decisions.
...
\textbf{Rationale:} A3 enforces the separation between authoritative (pre-execution) and observational (post-execution) data. A3 constrains normative admissibility of inputs; it does not determine whether an event exists in $V$. Without A3, execution outputs could influence decisions, violating determinism.
...
\textbf{Rationale:} A6 formalizes the pre-ontological nature of $L$. A6 determines event existence in $V$, while A3 determines normative admissibility of inputs to $G$. L filters information flow but does not make normative decisions. This allows constraint enforcement (rate limiting, schema validation) without event inflation in $V$.
...
From the axioms and definitions, immediate properties follow:
